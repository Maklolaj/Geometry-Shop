...
import {catchError, EMPTY, filter, map, Observable, of, subscribeOn, take, tap, timer} from 'rxjs';
        ...
        this.store.pipe(take(1)).subscribe( (store) => {
            if (store.products?.length === 0) {
                this.productService.getAllProducts().toPromise().then(products =>{
                    products ? this.store.dispatch(retrieveProductList({products})) : null
                })
            }
        })
    }

    waitForProfileDataToLoad(): Observable<any> { //should return Prodcut 
